...
{
    cis2_qmodel *qm = calloc(1, sizeof(cis2_qmodel));
    qm->base = m;
    ...
    cis2_quantize_matrix(m->embed_tokens, vocab, hidden, &qm->embed);
    qm->has_separate_lm_head = !m->cfg.tie_word_embeddings && m->lm_head != NULL;
    if (qm->has_separate_lm_head) {
        cis2_quantize_matrix(m->lm_head, vocab, hidden, &qm->lm_head);
    }

    qm->layers = calloc(m->cfg.num_hidden_layers, sizeof(cis2_qlayer));
    for (size_t i = 0; i < m->cfg.num_hidden_layers; i++) {
        const cis2_layer *L = &m->layers[i];
        cis2_qlayer *QL = &qm->layers[i];
        cis2_quantize_matrix(L->q_proj, hidden, hidden, &QL->q_proj);
        cis2_quantize_matrix(L->k_proj, nkv_dim, hidden, &QL->k_proj);
        cis2_quantize_matrix(L->v_proj, nkv_dim, hidden, &QL->v_proj);
        cis2_quantize_matrix(L->o_proj, hidden, hidden, &QL->o_proj);
        cis2_quantize_matrix(L->gate_proj, inter, hidden, &QL->gate_proj);
        cis2_quantize_matrix(L->up_proj, inter, hidden, &QL->up_proj);
        cis2_quantize_matrix(L->down_proj, hidden, inter, &QL->down_proj);
    }

    size_t nb = qm->embed.out_features * qm->embed.in_features + qm->embed.out_features * sizeof(float);
    if (qm->has_separate_lm_head)
        nb += qm->lm_head.out_features * qm->lm_head.in_features + qm->lm_head.out_features * sizeof(float);
    for (size_t i = 0; i < m->cfg.num_hidden_layers; i++) {
        cis2_qlayer *QL = &qm->layers[i];
        cis2_qmat *mats[7] = { &QL->q_proj, &QL->k_proj, &QL->v_proj, &QL->o_proj,
                                &QL->gate_proj, &QL->up_proj, &QL->down_proj };
        for (int j = 0; j < 7; j++) {
            nb += mats[j]->out_features * mats[j]->in_features + mats[j]->out_features * sizeof(float);
        }
    }
...
{
    if (!qm) return;
    cis2_qmat_free(&qm->embed);
    if (qm->has_separate_lm_head) cis2_qmat_free(&qm->lm_head);
    for (size_t i = 0; i < qm->base->cfg.num_hidden_layers; i++) {
        cis2_qlayer *QL = &qm->layers[i];
        cis2_qmat_free(&QL->q_proj); cis2_qmat_free(&QL->k_proj);
        cis2_qmat_free(&QL->v_proj); cis2_qmat_free(&QL->o_proj);
        cis2_qmat_free(&QL->gate_proj); cis2_qmat_free(&QL->up_proj);
        cis2_qmat_free(&QL->down_proj);
    }
    free(qm->layers);
    free(qm);
}
...
/* qmatvec_dispatch: use_int8 -> cis2_matvec_int8 (int32 accumulation);
 * !use_int8 -> cis2_matvec (fp32, the untouched reference path), so the
 * same driver code can produce both a bit-exact-int8 and an fp32 baseline
 * for direct comparison in cis2_eval_teacher_forced. */
static void qmatvec(int use_int8, const cis2_qmat *qw, const float *w_fp32,
                     ...
                     int8_t *qx_scratch)
{
    if (use_int8) {
        ...
        cis2_matvec_int8(qw, qx_scratch, x_scale, y);
    } else {
        cis2_matvec(w_fp32, x, y, out_features, in_features);
    }
}

/* Shared forward implementation used by both cis2_qrun_decode (last-position
 * logits only, matches the fp32 reference's cis2_run_decode contract) and
 * cis2_eval_teacher_forced (all-position streamed logits). If
 * `all_positions_cb` is non-NULL it is called once per position with that
 * position's logits (vocab floats); logits buffer is reused/overwritten
 * between calls, caller must not retain the pointer past the callback. */
typedef void (*logits_cb)(void *ctx, size_t pos, const float *logits, size_t vocab);

static void forward_generic(const cis2_model *m, const cis2_qmodel *qm, int use_int8,
                             ...
                             logits_cb cb, void *cb_ctx)
{
    ...
    for (size_t p = 0; p < ntok; p++) {
        if (use_int8) {
            const int8_t *qrow = qm->embed.q + (size_t)tokens[p] * hidden;
            float scale = qm->embed.scale[tokens[p]];
            for (size_t i = 0; i < hidden; i++) h[p * hidden + i] = (float)qrow[i] * scale;
        } else {
    ...
    for (size_t layer_i = 0; layer_i < m->cfg.num_hidden_layers; layer_i++) {
        ...
        const cis2_qlayer *QL = use_int8 ? &qm->layers[layer_i] : NULL;

        for (size_t p = 0; p < ntok; p++)
            cis2_rmsnorm(h + p * hidden, L->input_layernorm, eps, ln1 + p * hidden, hidden);

        for (size_t p = 0; p < ntok; p++) {
            qmatvec(use_int8, use_int8 ? &QL->q_proj : NULL, L->q_proj, ln1 + p * hidden, q + p * hidden, hidden, hidden, qx_scratch);
            if (L->q_bias) for (size_t i = 0; i < hidden; i++) q[p * hidden + i] += L->q_bias[i];
            qmatvec(use_int8, use_int8 ? &QL->k_proj : NULL, L->k_proj, ln1 + p * hidden, k + p * nkv_dim, nkv_dim, hidden, qx_scratch);
            if (L->k_bias) for (size_t i = 0; i < nkv_dim; i++) k[p * nkv_dim + i] += L->k_bias[i];
            qmatvec(use_int8, use_int8 ? &QL->v_proj : NULL, L->v_proj, ln1 + p * hidden, v + p * nkv_dim, nkv_dim, hidden, qx_scratch);
        ...
        for (size_t p = 0; p < ntok; p++) {
            qmatvec(use_int8, use_int8 ? &QL->o_proj : NULL, L->o_proj, attn_out + p * hidden, o + p * hidden, hidden, hidden, qx_scratch);
        ...
        for (size_t p = 0; p < ntok; p++) {
            qmatvec(use_int8, use_int8 ? &QL->gate_proj : NULL, L->gate_proj, ln2 + p * hidden, gate + p * inter, inter, hidden, qx_scratch);
            qmatvec(use_int8, use_int8 ? &QL->up_proj : NULL, L->up_proj, ln2 + p * hidden, up + p * inter, inter, hidden, qx_scratch);
            for (size_t i = 0; i < inter; i++) {
                hid[p * inter + i] = cis2_silu_pinned(gate[p * inter + i]) * up[p * inter + i];
            }
            qmatvec(use_int8, use_int8 ? &QL->down_proj : NULL, L->down_proj, hid + p * inter, down + p * hidden, hidden, inter, qx_scratch);
            for (size_t i = 0; i < hidden; i++) h[p * hidden + i] = h[p * hidden + i] + down[p * hidden + i];
        }
    }

    /* final norm + lm_head, per position needed (last only, or all via cb) */
    float *hn = malloc(hidden * sizeof(float));
    float *logits = malloc(vocab * sizeof(float));
    const cis2_qmat *qlm = use_int8 ? (qm->has_separate_lm_head ? &qm->lm_head : &qm->embed) : NULL;
    ...
    if (cb) {
        for (size_t p = 0; p < ntok; p++) {
            ...
            qmatvec(use_int8, qlm, lm_w, hn, logits, vocab, hidden, qx_scratch);
...
{
    eval_ctx *ctx = vctx;
    if (pos + 1 >= ctx->ntok) return; /* no target for the last position */
                               ...
                               uint32_t *pred_ids_out)
{
    ...
    forward_generic(m, qm, use_int8, tokens, ntok, NULL, eval_cb, &ctx);
