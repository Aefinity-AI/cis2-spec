...
check_fails "flipped chain digest byte" \
  "$BINDIR/cis-verify" "$tmp/t1.receipt" model/MODEL.SAF model/EMBED.BIN model/VOCAB.BIN
...
check_fails "edited ctx= argument" \
  "$BINDIR/agent_trace" verify model/MODEL.SAF model/EMBED.BIN model/VOCAB.BIN "$tmp/t2.receipt" --table receipts/chain.tsv
...
check_fails "dropped step + kept old trace-chain" \
  "$BINDIR/agent_trace" verify model/MODEL.SAF model/EMBED.BIN model/VOCAB.BIN "$tmp/t3.receipt" --table receipts/chain.tsv
...
check_fails "relabelled host line" \
  "$BINDIR/agent_trace" verify model/MODEL.SAF model/EMBED.BIN model/VOCAB.BIN "$tmp/t4.receipt" --table receipts/chain.tsv
