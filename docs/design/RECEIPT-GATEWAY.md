...
**Extended run.** A larger adversarial evaluation reused a separate 60-item real legitimate-episode corpus and added 20 hand-constructed injected attacks spanning the same 5 deny paths (5 replay, 5 output-byte tamper, 5 strict-grounding violation, 3 off-allowlist artifact triple, 2 verify-execute binding mismatch), submitted through one live gateway process alongside the 60 clean items. Result: 60/60 clean ALLOW and 20/20 injected DENY, each for the verbatim-correct reason (e.g. `freshness: (session, counter, action-hash) already seen` for replay, `artifact triple not on allowlist` for the decoy triple). This shows the gate denies every injected attack class for the right reason while passing all legitimate work, on hand-constructed injections, not adversarially optimised ones — it does not demonstrate robustness against an adversary who optimizes attacks against the gate itself.
