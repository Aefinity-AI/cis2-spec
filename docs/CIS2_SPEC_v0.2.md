...
third parties can attempt bit-for-bit reproduction (see this repository's
README). This document is normative for the scope stated below: a
...
This is v0.2 of the spec first drafted as v0.1 (`docs/CIS2_SPEC_v0.1.md`).
v0.2 closes the three gaps v0.1 left explicitly open in its own §14/Appendix
B (table digests not bound into the receipt, RoPE `inv_freq` pinned only for
one `rope_theta`, and an unpinned hex-ASCII-vs-raw-bytes ambiguity in the
witness header). **Every value in v0.1's §13 test vectors changed as a
result** (`CIS2_REF` and `table_digest`); §16 is a full changelog. This
document's body is, as with v0.1, written to be implementable **without**
access to `src/` — Appendix A cites exact `file:line`s only so a reviewer can
audit that this spec did not silently diverge from what the reference
actually computes.
...
### 6.3 sin/cos — route (b), pinned Taylor series after 2π range reduction

Range reduction to `r ∈ [-π, π]` (2-part-π split, Cephes pattern, strict
left-to-right, no FMA):

```
k    = round(x / TWO_PI_HI)     # round-to-nearest integer, ties per IEEE round()
khi  = k * TWO_PI_HI
r0   = x - khi
klo  = k * TWO_PI_LO
r    = r0 - klo
...
```
TWO_PI_HI = 0x40C90FDA   (== 6.283185)
TWO_PI_LO = 0x347FF14D   (== 2.383651e-07; 2π − TWO_PI_HI, exact-in-f64 split)
```

`cos(x)`: Horner in `r² `, descending from the highest-degree coefficient,
strict left-to-right, no FMA:

```
r2   = r * r
poly = COS_COEF[10]
for i in 9 downto 0:
    poly = poly * r2 + COS_COEF[i]
return poly
```

`sin(x)`: identical structure with `SIN_COEF`, then one final multiply by
`r`:
...
poly = SIN_COEF[10]
for i in 9 downto 0:
    poly = poly * r2 + SIN_COEF[i]
return poly * r
```

Pinned coefficients (`1/(2n+1)!` for sin, `1/(2n)!` for cos, `n = 0..10`,
each independently rounded once to `f32` from the exact rational — not
copied from any specific published binary, but algorithmically identical
to the standard Taylor coefficients):

```
SIN_COEF[0..10] = 0x3F800000, 0xBE2AAAAB, 0x3C088888, 0xB9500D01,
                  0x3638EF1E, 0xB2D7322C, 0x2F30922E, 0xAB573FA0,
                  0x274A963A, 0xA317A4DA, 0x1EB8DC77

COS_COEF[0..10] = 0x3F800000, 0xBF000000, 0x3D2AAAAC, 0xBAB60B62,
                  0x37D00D02, 0xB493F27E, 0x310F76C9, 0xAD49CBAA,
                  0x29573F9E, 0xA53413C5, 0x20F2A15F
```
...
### 6.6 Table digest (NOW BOUND into the witness chain — see §12.1, closes v0.1 §14.3)
...
`[TWO_PI_HI, TWO_PI_LO]`, then `SIN_COEF[0..11]`, then `COS_COEF[0..11]`,
then **(new in v0.2)** `LOG_SQRTHF`, then `LOG_Q1`, then `LOG_Q2`, then
`LOG_P[0..9]`, each as 4 little-endian bytes of the `f32` bit pattern,
concatenated, one continuous SHA-256 stream:

```
table_digest = 465d358ccd63721256dbd2abbc77ad5de755adf3230635f95b12b1727dfa1ea3
```

**v0.1 vs v0.2**: the *set* of coefficients hashed changed (the `ln`
table's four groups are new), so this digest differs from v0.1's
`0bf9257bc5...` for that reason alone, **independent of** the raw-bytes-vs-
hex-ASCII change (§12.1) — the value above is the coefficient-table digest
computed with the same "concatenate f32 LE bytes" method v0.1 used, just
over a longer coefficient list. Unlike v0.1 (§6.5's "informative binding,
NOT part of the witness chain"), this digest **is now** one of the raw
32-byte inputs folded directly into `CIS2_REF` — see §12.1.

### 6.7 Accuracy characterization (informative, new in v0.2 addendum — closes the "pinned by fiat" gap flagged in `docs/paper/03_contract.md`)

§6.2–§6.5 pin exact coefficients but, prior to this addendum, did not
quantify their error vs. a correctly-rounded fp32 reference. That
characterization has now been done — full method, oracle, and domain
derivation in `docs/H2_TRANSCENDENTAL_RIGOR.md` — and is summarized here.
**No coefficient in §6.2–§6.5 changed; `table_digest()` (§6.6) is
unaffected.** Determinism does not depend on correctness of these values,
only on their being pinned (fixed bit-exact fp32 arithmetic, verified by
`table_digest()`); accuracy vs. IEEE-754 correctly-rounded is bounded, per
function, over the domain this spec's pinned test vectors actually
exercise, as follows:

| function | domain exercised by §13's pinned test vectors | max ULP vs. correctly-rounded fp32 | max relative error |
|---|---|---|---|
| `rsqrt_cr` | all finite x>0 | 0 (correctly rounded by construction — composed of two IEEE-754-mandatory correctly-rounded ops) | 0 |
| `exp_pinned` | x∈[-40,40] (softmax post-max-sub args ≤0, SiLU gate args, RoPE inv_freq exponents) | 1 | 1.19e-7 |
| `ln_pinned` | x = rope_theta only, called once per model load | 0 at the two pinned values (100000.0, 1000000.0); ≤1 over a padded scan | 0 / 7.85e-8 |
| `sin_pinned` | RoPE angle = pos·inv_freq[i], pos≤19 (§13's 20-position decode) | up to 1104 (SmolLM2, θ=1e5) | 1.00e-4 |
| `cos_pinned` | same | up to 398 (SmolLM2, θ=1e5) | 4.19e-5 |

**Finding (accuracy concern, not a determinism concern):** `sin_pinned`/
`cos_pinned` exceed a 4-ULP threshold even on the exact grid the pinned
test vectors use. Root cause: `reduce_2pi`'s two-constant
(`TWO_PI_HI`/`TWO_PI_LO`) range reduction is done entirely in f32 (per
this spec's no-FMA, fp32-only constraint, §3.2), which suffers
catastrophic cancellation growing linearly with the position index — not
a polynomial-degree inadequacy. The function remains exactly
bit-reproducible on any conformant IEEE-754 fp32 ISA; what is not true is
that those bits are close to the mathematically correct value at better
than ~1e-4 relative error for this spec's 16-token pinned decode, and the
error grows to ~0.58 relative (unusable) if decode length were extended
toward `max_position_embeddings = 8192`. Any future extension of this
spec family's pinned decode length should re-run
`docs/H2_TRANSCENDENTAL_RIGOR.md`'s characterization before assuming the
current reduction/polynomial is adequate at the new length.
...
```
CIS2_REF = a0c563ef804f50413b7fb6619ae4afe9b51b1ffa7655e944221393e85d6261da
```

printed as `CIS2_REF digest=a0c563ef... prompt_idx=0 prompt_toks=4
gen_toks=16 dtype=fp32`. **This digest is not backward-compatible with
v0.1's `ba88708bf4...`** — both the item order (table digests now feed the
chain first) and the item 3–5 encoding (raw bytes, not hex-ASCII) changed;
see §16 for the full breakdown of which change contributed how.
...
inv_freq_table_digest  = da9f6dcfde0425588815509e874515cdcd3d6b8818b6d0136590052e7bbf6f12
argmax_digest          = 0b9c8f3ac90d0b9cd5f1719ac327dca1fc639fd87468305fccebbe3d56f67aff
CIS2_REF (witness_digest) = a0c563ef804f50413b7fb6619ae4afe9b51b1ffa7655e944221393e85d6261da
```

Note `generated_token_ids` and `argmax_digest` are **unchanged from v0.1**
— none of v0.2's three changes altered the actual computed logits or
greedy decisions, only the receipt's hashing (§16).
