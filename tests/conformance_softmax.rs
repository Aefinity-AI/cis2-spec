//! Self-test for `tests/conformance/vectors/softmax_v1.txt` (E21 sixth
//! slice: softmax). Runs the CIS-2 reference pinned softmax
//! (docs/CIS2_SPEC_v0.2.md §9.3, same code as `src/math.rs::softmax_seq`,
...
//! digest against `tests/conformance/vectors/softmax_v1.expected`.
//!
...
#[test]
fn softmax_v1_reproduces_pinned_digest() {
    let dir = Path::new(env!("CARGO_MANIFEST_DIR")).join("tests/conformance/vectors");
    let vector_text = fs::read_to_string(dir.join("softmax_v1.txt")).unwrap();
    let expected_text = fs::read_to_string(dir.join("softmax_v1.expected")).unwrap();

    let n: usize = parse_field(&vector_text, "n").trim().parse().unwrap();
    let mut scores = parse_bits_list(parse_field(&vector_text, "scores_bits"));
    assert_eq!(scores.len(), n);

    math::softmax_seq(&mut scores);
    let got_digest = digest_f32_le(&scores);
    let got_bits: Vec<u32> = scores.iter().map(|f| f.to_bits()).collect();
    ...
    assert_eq!(
        ...
        "softmax_v1 output bit patterns diverged from tests/conformance/vectors/softmax_v1.expected"
    );
    assert_eq!(
        got_digest, expected_digest,
        "softmax_v1 digest diverged from pinned value in tests/conformance/vectors/softmax_v1.expected"
    );
}
...
fn print_bits_for_generation() {
    ...
    // `cargo test --test conformance_softmax print_bits -- --ignored --nocapture`
    // to regenerate the fixture if the reference math ever changes.
    let dir = Path::new(env!("CARGO_MANIFEST_DIR")).join("tests/conformance/vectors");
    let vector_text = fs::read_to_string(dir.join("softmax_v1.txt")).unwrap();
    let mut scores = parse_bits_list(parse_field(&vector_text, "scores_bits"));
    math::softmax_seq(&mut scores);
    let bits: Vec<String> = scores.iter().map(|f| format!("0x{:08X}", f.to_bits())).collect();
    println!("out_bits={}", bits.join(","));
    println!("out_sha256={}", digest_f32_le(&scores));
}
